...
// MatrixXcf normal(const int rows, const int cols) {
//     MatrixXcf n(rows, cols);
//     normal_distribution<float> nd(0.0, 1.0);
//     random_device rd;
//     mt19937 gen(rd());
...
// MatrixXcf orthogonal(const MatrixXcf &M) {
//     MatrixXcf thinQ = MatrixXcf::Identity(M.rows(), M.cols());
//     MatrixXcf thinR = MatrixXcf::Identity(M.cols(), M.rows());

//     MatrixXcf flattened = normal(M.rows(), M.cols());
//     flattened = flattened.real().cast<complex<float>>();
//     HouseholderQR<MatrixXcf> qr(flattened);
//     MatrixXcf q = qr.householderQ() * thinQ;
//     MatrixXcf r = thinR * qr.matrixQR().triangularView<Upper>();
//     auto d = r.diagonal();
//     q *= d.cwiseSign();
//     return q;
// }

// pair<MatrixXcf, MatrixXcf> LSI_approximation(const MatrixXcf &M, const int k, const float epsilon = 1e-6) {
//     auto c = M.cols();
//     MatrixXcf R(c, k);
//     R.setZero();
//     MatrixXcf orth_R = orthogonal(R);
//     MatrixXcf B = sqrt(c * 1.0 / k) * orth_R.adjoint() * M;

//     JacobiSVD<MatrixXcf> svd(B * B.adjoint(), ComputeThinU | ComputeThinV);
//     MatrixXcf u = svd.matrixU();
//     MatrixXcf s = svd.singularValues();

//     MatrixXcf new_s = s.cwiseSqrt();
//     new_s.array() += epsilon;
//     MatrixXcf div = u.array() / new_s.array();
//     MatrixXcf v = B.adjoint() * div;

//     return make_pair(M * v, v);
// }

// MatrixXcf outer_products(const MatrixXcf &A, const MatrixXcf &B) {
//     return A * B.adjoint();
// }

// MatrixXcf ele_mul(const MatrixXcf &A, const MatrixXcf &B) {
//     if (A.cols() != B.cols()) throw std::invalid_argument("mismatched rows");

//     MatrixXcf m(B.rows(), B.cols());
...
// void normalize(MatrixXcf &M) {
//     for (int i = 0; i < M.rows(); ++i) {
//         complex<float> norm = M.row(i).norm();
//         for (int j = 0; j < M.cols(); ++j)
//             M(i, j) /= norm;
//     }
// }

// pair<MatrixXcf, MatrixXcf> dim2_eig(MatrixXcf &M) {
//     complex<float> det = M(0, 0) * M(1, 1) - M(1, 0) * M(0, 1);
//     complex<float> trace = M(0, 0) + M(1, 1);
//     MatrixXcf eigenvalues(1, 2), eigenvectors(2, 2);
//     eigenvalues(0, 0) = trace / complex<float>(2, 0) + sqrt(trace * trace / complex<float>(4, 0) - det);
//     eigenvalues(0, 1) = trace / complex<float>(2, 0) - sqrt(trace * trace / complex<float>(4, 0) - det);
//     eigenvectors << eigenvalues(0, 0) - M(1, 1), eigenvalues(0, 1) - M(1, 1), M(1, 0), M(1, 0);
...
// // grad_u和u的大小是d*d的一维数组
// // projUNN-T

// // u和grad_u转化为Matrix
//     Map<MatrixXf> U_(u, d, d);
//     MatrixXf G_ = Map<MatrixXf>(grad_u, d, d);
//     MatrixXcf U = U_.cast<complex<float>>().transpose();
//     MatrixXcf G = G_.cast<complex<float>>().transpose();

//     // get k
//     const int k = 1; //TODO: low_rank approximation

//     //LSI算出a,b
//     auto pair_a_b = LSI_approximation(G, k);
//     MatrixXcf a = pair_a_b.first;
//     MatrixXcf b = pair_a_b.second;

//     //projUNN-T for update
//     MatrixXcf a_hat = U.adjoint() * a;

//     MatrixXcf a_and_b(b.rows(), b.cols() + a_hat.cols());
//     a_and_b.leftCols(b.cols()) = b;
//     a_and_b.rightCols(a_hat.cols()) = a_hat;

//     MatrixXcf thinQ = MatrixXcf::Identity(a_and_b.rows(), a_and_b.cols());
//     HouseholderQR<MatrixXcf> qr(a_and_b);
//     MatrixXcf q = qr.householderQ() * thinQ;
//     MatrixXcf projectors = q.adjoint();

//     MatrixXcf a_obasis = projectors * a_hat;
//     MatrixXcf b_obasis = projectors * b;

//     MatrixXcf sub_arr = 0.5 * outer_products(a_obasis, b_obasis) - 0.5 * outer_products(b_obasis, a_obasis);
//     int cols = sub_arr.cols();
//     MatrixXcf eigenvalues(1, cols), eigenvectors(cols, cols);
//     if (cols == 2) {
//         auto pair_eig = dim2_eig(sub_arr);
//         eigenvalues = pair_eig.first;
//         eigenvectors = pair_eig.second;
//     } else {
//         ComplexEigenSolver<MatrixXcf> eigenSolver(sub_arr);
//         eigenvalues = eigenSolver.eigenvalues().transpose();
//         eigenvectors = eigenSolver.eigenvectors();
//     }
//     MatrixXcf s = eigenvalues.array().expm1();

//     MatrixXcf v = q * eigenvectors;
//     MatrixXcf update = outer_products(U * (ele_mul(s, v)), v);
//     MatrixXf update_ = update.real();

//     // update u
//     U_ += update_.transpose();
//     // cout << U_ * U_.adjoint() << endl;
// }